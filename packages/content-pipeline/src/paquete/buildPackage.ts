...
function insertarInfracciones(
  ...
  const stmtInf = db.prepare(
    `INSERT INTO infraccion (
       ...
       importe_max_eur, puntos, pena_texto, gravedad_penal, texto_boletin, variantes_boletin,
       competencia, ambito, territorio_id, desplaza_id, origen, valid_from, valid_to,
       estado_revision, nota_revision
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  );
